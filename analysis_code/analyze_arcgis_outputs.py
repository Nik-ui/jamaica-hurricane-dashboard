...
if (forecastFocus.length) {
  ...
  const forecastCallout = el("text", {x: fcCalloutX + 18, y: fcCalloutY - 16, class: "trackCallout"});
  forecastCallout.textContent = "48-hour model forecast track";
  const forecastSub = el("text", {x: fcCalloutX + 18, y: fcCalloutY + 1, class: "trackCalloutSub"});
  forecastSub.textContent = "dashed red line; research visualization, not official forecast";
}
