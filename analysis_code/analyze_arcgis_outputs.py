...
if (forecastFocus.length) {
  ...
  const forecastCallout = el("text", {x: fcCalloutX - 120, y: fcCalloutY - 18, class: "trackCallout"});
  forecastCallout.textContent = "48-hour model forecast track";
  const forecastSub = el("text", {x: fcCalloutX - 120, y: fcCalloutY - 2, class: "trackCalloutSub"});
  forecastSub.textContent = "dashed red line; not official";
}
